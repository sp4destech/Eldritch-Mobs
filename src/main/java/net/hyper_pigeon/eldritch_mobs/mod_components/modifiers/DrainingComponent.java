...
public class DrainingComponent implements ModifierInterface {
        ...
        if(entity.getAttacking() != null && entity.getTarget() != null && entity.getTarget().isAlive()) {
            ...
            if (time > nextAbilityUse) {
                ...
                if(EldritchMobsMod.CONFIG.intensity <= 1) {
                    target.addStatusEffect(new StatusEffectInstance(StatusEffects.MINING_FATIGUE, 125, 0));
                }
                else if(EldritchMobsMod.CONFIG.intensity == 2) {
                    target.addStatusEffect(new StatusEffectInstance(StatusEffects.MINING_FATIGUE, 150, 0));
                }
                else {
                    target.addStatusEffect(new StatusEffectInstance(StatusEffects.MINING_FATIGUE, 175, 0));
                }
